...
contract VotingEscrow {
    ...
    function _checkpoint(
        ...
        if (addr != address(0)) {
            ...
            if (old_locked.end > block.timestamp && old_locked.amount > 0) {
                ...
                u_old.bias = u_old.slope.mul(
                    int128(old_locked.end.sub(block.timestamp))
                );
            }
            if (new_locked.end > block.timestamp && new_locked.amount > 0) {
                //s: u_new.slope = new_locked.amount / MAXTIME;
                u_new.slope = new_locked.amount.div(int128(MAXTIME));
                u_new.bias = u_new.slope.mul(
                    int128(new_locked.end.sub(block.timestamp))
                );
            }
        ...
        for (uint256 i; i < 255; i++) {
            ...
            last_point.bias = last_point.bias.sub(
                last_point.slope.mul(int128(t_i.sub(last_checkpoint)))
            );
        ...
        if (addr != address(0)) {
            ...
            last_point.slope = last_point.slope.add(
                u_new.slope.sub(u_old.slope)
            );
    ...
    function _deposit_for(
        ...
        LockedBalance memory _locked =
            LockedBalance(locked_balance.amount, locked_balance.end);
        LockedBalance memory old_locked =
            LockedBalance(locked_balance.amount, locked_balance.end);
        ...
        returns (uint256)
    {
        ...
        if (_epoch == 0) {
            ...
            last_point.bias = last_point.bias.sub(
                last_point.slope.mul(int128(_t.sub(last_point.ts)))
            );
            if (last_point.bias < 0) {
                last_point.bias = 0;
            }
        ...
        returns (uint256)
    {
        ...
        if (_.d_block != 0) {
            block_time = block_time.add(
                _.d_t.mul(_block.sub(point_0.blk)).div(_.d_block)
            );
        }

        upoint.bias = upoint.bias.sub(
            upoint.slope.mul(int128(block_time.sub(upoint.ts)))
        );
        if (upoint.bias >= 0) {
    ...
    event DEBUG(
        int128 last_point_bias,
        int128 last_point_slope,
        uint256 t_i,
        uint256 last_point_ts
    );
        ...
        returns (uint256)
    {
        ...
        for (uint256 i; i < 255; i++) {
            ...
            last_point.bias = last_point.bias.sub(
                last_point.slope.mul(int128(t_i.sub(last_point.ts)))
            );
    ...
    function totalSupply(uint256 t) external returns (uint256) {
        //s: uint256 t = block.timestamp
        /***
    ...
    function totalSupplyAt(uint256 _block) external returns (uint256) {
        ...
        if (target_epoch < _epoch) {
            ...
            if (point.blk != point_next.blk) {
                dt = (_block.sub(point.blk))
                    .mul(point_next.ts.sub(point.ts))
                    .div(point_next.blk.sub(point.blk));
            }
        } else {

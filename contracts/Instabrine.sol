...
contract Instabrine {
        ...
        returns (uint256)
    {
        ...
        INeuronPool(pickleJar).depositAll();

        // Refund msg.sender
        uint256 _jar = INeuronPool(pickleJar).balanceOf(address(this));
        INeuronPool(pickleJar).transfer(msg.sender, _jar);
    ...
    function primitiveToPickleJar(
        ...
        INeuronPool(jar).deposit(amount);
        
        uint256 _jar = INeuronPool(jar).balanceOf(address(this));
    ...
    function curvePickleJarToPrimitive_1(
        ...
        INeuronPool(pickleJar).withdraw(amount);

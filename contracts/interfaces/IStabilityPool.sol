...
interface IStabilityPool {
    ...
    function deposits(address)
        external
        view
        returns (uint256 initialValue, address frontEndTag);

    function epochToScaleToG(uint128, uint128) external view returns (uint256);

    function epochToScaleToSum(uint128, uint128)
        external
        view
        returns (uint256);
    ...
    function frontEnds(address)
        external
        view
        returns (uint256 kickbackRate, bool registered);

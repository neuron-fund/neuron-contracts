    ...
    RewardsDistributionRecipient
{
    ...
    function _checkpoint(
        ...
        if (_addr != address(0)) {
            ...
            if (_newLocked.end > block.timestamp && _newLocked.amount > 0) {
                ...
                userNewPoint.bias = userNewPoint.slope.mul(
                    int128(_newLocked.end.sub(block.timestamp))
                );
            }
        ...
        if (block.timestamp > lastPoint.ts) {
            blockSlope = StableMath
                .scaleInteger(block.number.sub(lastPoint.blk))
                .div(block.timestamp.sub(lastPoint.ts));
        }
        ...
        for (uint256 i = 0; i < 255; i++) {
            ...
            int128 biasDelta =
                lastPoint.slope.mul(int128(iterativeTime.sub(lastCheckpoint)));
            ...
            lastPoint.blk = initialLastPoint.blk.add(
        ...
        if (_addr != address(0)) {
            ...
            lastPoint.slope = lastPoint.slope.add(
                userNewPoint.slope.sub(userOldPoint.slope)
            );
            lastPoint.bias = lastPoint.bias.add(
                userNewPoint.bias.sub(userOldPoint.bias)
            );
            if (lastPoint.slope < 0) {
                lastPoint.slope = 0;
            }
    ...
    ****************************************/
    ...
    function balanceOf(address _owner) public view override returns (uint256) {
        ...
        lastPoint.bias = lastPoint.bias.sub(
            lastPoint.slope.mul(int128(block.timestamp.sub(lastPoint.ts)))
        );
        if (lastPoint.bias < 0) {
            lastPoint.bias = 0;
        }
        ...
        returns (uint256)
    {
        ...
        if (dBlock != 0) {
            ...
            blockTime = blockTime.add(
                dTime.mul(_blockNumber.sub(point0.blk)).div(dBlock)
            );
        }
        // Current Bias = most recent bias - (slope * time since update)
        upoint.bias = upoint.bias.sub(
            upoint.slope.mul(int128(blockTime.sub(upoint.ts)))
        );
        if (upoint.bias >= 0) {
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < 255; i++) {
            ...
            lastPoint.bias = lastPoint.bias.sub(
                lastPoint.slope.mul(int128(iterativeTime.sub(lastPoint.ts)))
            );
            if (iterativeTime == _t) {
                break;
            }
        ...
        returns (uint256)
    {
        ...
        if (targetEpoch < epoch) {
            ...
            if (point.blk != pointNext.blk) {
                dTime = (_blockNumber.sub(point.blk))
                    .mul(pointNext.ts.sub(point.ts))
                    .div(pointNext.blk.sub(point.blk));
            }
    ...
    ****************************************/
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        uint256 rewardUnitsToDistribute =
            rewardRate.mul(lastTimeRewardApplicable().sub(lastUpdateTime));
    ...
    function earned(address _addr) public view override returns (uint256) {
        ...
        uint256 userNewReward =
            staticBalanceOf(_addr).mulTruncate(userRewardDelta);

...
describe('GaugesDistributor.sol', () => {
  ...
  beforeEach(async () => {
    ...
    gaugesDistributor = (await ethers.getContractAt(
      'GaugesDistributor',
      GaugesDistributorDeployment.address
    ...
    neuronPoolToken = (await ethers.getContractAt(
      '@openzeppelin/contracts/token/ERC20/IERC20.sol:IERC20',
      await neuronPool.token()
    )) as IERC20

    const uniswapRouter = (await ethers.getContractAt('IUniswapRouterV2', UNISWAP_ROUTER_V2)) as IUniswapRouterV2
    await uniswapRouter
      .connect(donor)
      .swapExactETHForTokens('0', [WETH, CRV3], await donor.getAddress(), Date.now() + 30000, {
        gasLimit: 4000000,
  ...
  it(`Distribute: regular test`, async () => {
    ...
    assert(
      resultBalance.gte(lowLevel) && resultBalance.lte(topLevel),
      `Result balance = ${resultBalance}; topLevel = ${topLevel}, lowLevel = ${lowLevel}`
    )
  })
  ...
  it(`Distribute: 3 users regular test`, async () => {
    ...
    for (let i = 0; i < testUsers.length; i++) {
      ...
      assert(
        rewardBalance.gte(lowLevel) && rewardBalance.lte(topLevel),
        `Result balance ${i} = ${rewardBalance}; topLevel = ${topLevel}, lowLevel = ${lowLevel}`
      )
    }
  })
  ...
  it(`Distribute: timewindow test`, async () => {
    ...
    assert(
      resultBalance2.gte(lowLevel) && resultBalance2.lte(topLevel),
      `Result balance2 = ${resultBalance2}; topLevel = ${topLevel}, lowLevel = ${lowLevel}`
    )
  })
})

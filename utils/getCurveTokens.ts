...
export const get3Crv = async (recipient: Signer) => {
  ...
  await uniswapRouter.swapExactETHForTokens('0', [WETH, DAI], await recipient.getAddress(), Date.now() + 30000, {
    gasLimit: 4000000,
...
export const getRenCrv = async (recipient: Signer) => {
  ...
  await uniswapRouter.swapExactETHForTokens('0', [WETH, WBTC], await recipient.getAddress(), Date.now() + 30000, {
    gasLimit: 4000000,
...
export const getFeiTribe = async (recipient: Signer) => {
  ...
  await uniswapRouter.swapETHForExactTokens(tokensAmount, getFeiPath, await recipient.getAddress(), Date.now() + 60, {
    value: tokensAmount,
  })
  await uniswapRouter.swapETHForExactTokens(tokensAmount, getTribePath, await recipient.getAddress(), Date.now() + 60, {
    value: tokensAmount,
  })
  ...
  await uniswapRouter.addLiquidity(FEI, TRIBE, feiBalanceAfter, tribeBalanceAfter, 0, 0, accAddress, Date.now() + 30000)
